...
# Chart Drawing:
In this part, you need to draw a stacked area chart. Based on the provided CSV-format data, each row represents the incremental values of different categories for that year. The stacking order of the data series in the stacked area chart must strictly follow the series order given in the first row and must not be changed. Make sure the chart is drawn accurately, with the data correctly mapped to the horizontal and vertical axes. A reference chart for the desired result is shown as follows. The chart must include all the provided data and labels, but do not display numeric values directly on the chart. Use a consistent visual style throughout, with the specific style decided by you. The color of the chart is determined by the title, and the chart background should be the same as the underlying canvas background, with an appropriate color chosen according to the title. The entire graphic should use only a single background color.
# Layout Information:
...
image_path = "csv/stack_area/steack_area.png"
example = encode_image_to_base64(image_path)
